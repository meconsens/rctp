...
library(cowplot)
```
Let's get the genotype SNP data associated with the ROSMAP individuals, specifically for the TMEM106B SNP.
```{r}
setwd('./cohortMetadata')
#get marker list
SNP_data <-  read.table("TMEM106B_ROSMAP.txt", sep = " ", fill = TRUE )
setwd('../')


SNP_data <- t(SNP_data)
SNP_HET <- data.frame("gwas_id" = SNP_data[-c(1:4), -2], "TMEM106B" = as.factor("HET"))
                     
SNP_HOM <- data.frame("gwas_id" = SNP_data[-c(1:4), -1], "TMEM106B" = as.factor ("HOM"))
SNP_HOM <-SNP_HOM[!(is.na(SNP_HOM$gwas_id) | SNP_HOM$gwas_id==""), ]

SNP_meta <- rbind(SNP_HET, SNP_HOM)
```
Let's plot some box-plots for TMEM106B heterozygosity vs. homozygosity.
```{r}
#get the metadata
setwd('./cohortMetadata')
...
#add in SNP data to metadata
ROSID_SNP <- merge(ROSID, SNP_meta, by = "gwas_id")
ROSID_SNP <- ROSID_SNP[, c("projid","gwas_id", "TMEM106B")]
ROS_meta <- merge(ROS_meta,ROSID_SNP, by = "projid")
ROS_meta <- setnames(ROS_meta, old = c('projid'), new = c('subject'))
...
boxplot_data <- unique(merge(merged, ROS_meta, by = "subject"))

boxplot_SST <- boxplot_data %>% 
    ggplot(aes(x = TMEM106B, y = SST)) + theme_minimal()+
    #geom_violin(alpha=0.4) + 
    geom_quasirandom(size=0.6,shape=19,aes(col=TMEM106B),alpha=0.8) +
    stat_summary(fun=mean,geom="point",aes(fill=TMEM106B),size=4,shape=23,col="black")+
    scale_fill_brewer(palette="Set1")+
    scale_color_brewer(palette="Set1")+
    ggtitle('SST rCTPs association with \n TMEM106B heterozygosity vs. homozygosity')+
    ylab(' SST rCTP') + 
    xlab('') 

boxplot_IT <- boxplot_data %>% 
    ggplot(aes(x = TMEM106B, y = IT)) + theme_minimal()+
    #geom_violin(alpha=0.4) + 
    geom_quasirandom(size=0.6,shape=19,aes(col=TMEM106B),alpha=0.8) +
    stat_summary(fun=mean,geom="point",aes(fill=TMEM106B),size=4,shape=23,col="black")+
    scale_fill_brewer(palette="Set1")+
    scale_color_brewer(palette="Set1")+
    ggtitle('IT rCTPs association with \n TMEM106B heterozygosity vs. homozygosity')+
    ylab( ' IT rCTP') + 
    xlab('')  
...
#TMEM106B 0,1,2 categorical variable 3 possible variables homozgyous recessive, heterozygous, homozygous dominant, plot the volcano plots cell type proportions with covariance for TMEM106B
...
print(full_plot)
```
Alright let's try this again with the newest marker list :)
```{r}
marker <- c("subclass_new_common_final")
setwd('./commonMarkerLists')
#get marker list
subclass_new <-  readRDS(paste0(marker, '.rds'))
setwd('../')

#get MGP, aka the rCTPs
mgp_name <-"mgp_ROSMAP"
setwd('./mgpResults_subclass_new')
ROS_mgp <- readRDS(paste0(mgp_name, ".rds"))
ROSMGP <- ROS_mgp$model
setwd('../')
...
merged <- merge(conversion, ROSMGP, by="projid")
merged <- setnames(merged, old = c('projid','subject'), new = c('subject','projid'))
relevant_merge <- data.frame("projid"= merged$projid, "SST" = merged$SST, "L2/3 IT"= merged$`L2/3 IT`)
mod_info <- merge(combined_data, relevant_merge, by="projid")
mod_info <- unique(mod_info)
...
for(model in model_names){
  ...
  mod_info[paste0(model, "cog_res")] <- resid(mod_cog)
  if(model=="BASELINE"){
    model_data_new <- data.frame("model" = model, "r2" = mod_cog$stats['R2'], "optimisim" = validation_data[1,4], "corrR2" =validation_data[1,5], "p.val" = NA)
    
  }
  else{
    significance <- (anova(BASELINEcog_ANOVA_model, mod_for_anova)[6])[2,1]
    temp = data.frame("model" = model, "r2" = mod_cog$stats['R2'], "optimisim" = validation_data[1,4], "corrR2" =validation_data[1,5], "p.val" = significance)
    model_data_new <- rbind(model_data_new, temp)
  }
  
}

model_data_new$FDR <- p.adjust(model_data_new$p.val, method="fdr")
model_data_new$significant <- ifelse(model_data_new$FDR < 0.05, "**", "")
model_data_new$bonf <- p.adjust(model_data_new$p.val, method="bonf")
model_data_new$significant_bonf <- ifelse(model_data_new$bonf < 0.05, "***", "")
```
Let's plot.

```{r}
cell_type_names  <- (names(subclass_new_common_final))
...
model_data_new$model <- gsub(".", '+', model_data_new$model, fixed = T)
model_data_new$model <- gsub("L2+3+IT", 'L2.3.IT', model_data_new$model, fixed = T)

model_data_new$model <- factor(model_data_new$model, levels = rev(c("BASELINE", "APOE4D", 
                                                          "APOE2D", "L2.3.IT",
                                                          "APOE4D+L2.3.IT",
                                                          "APOE2D+L2.3.IT","SST",
                                                          "APOE2D+SST",
                                                          "APOE4D+SST", "L2.3.IT+SST")))
model_data_new <- arrange(model_data_new, model)

model_data_new$range <- 
  model_data_new$r2 - model_data_new$r2[length(model_data_new$r2)]
model_data_new$percent <- 
  as.numeric(format(round(model_data_new$range*100, 2), nsmall = 2))
model_data_new$raw_percent <- 
  as.numeric(format(round(model_data_new$r2*100, 2), nsmall = 2))
#remove APOE2D info
model_data_new <- model_data_new[-c(3, 5, 8),]

model_plot = model_data_new %>% 
  ggplot(aes(x = model, y = percent ,fill = "#E6E6FA")) + 
  scale_fill_manual(values = "#967bb6") +
  scale_y_continuous(expand = c(0,0))+
  geom_bar(stat = "identity", show.legend = FALSE) + 
  ylab(paste0('Additional % Variance Explained \n Beyond Baseline ', format(round(model_data_new$r2[length(model_data_new$r2)]*100)), '% R2')) + 
  xlab('Model for Residual Cogniton') +  coord_flip()
...
setwd('./figures')
saveRDS(full_plot, "figure4-new.rds")
pdf(file = "figure4-new.pdf", 
    width = 10, # The width of the plot in inches
    height = 12)
print(full_plot)
dev.off()
