...
list_name <- paste0(marker, "_common_final.rds")
subclass_CgG_MTG <- readRDS(list_name)
setwd('../')


mgp_name <- paste0("mgp_ROSMAP")
setwd(paste0('./mgpResults_',marker))
...
pathology_vars <- ROS_meta %>% dplyr::select("projid", all_of(var_names))
...
heatmap <- ggplot(results, aes(celltype, pathology, fill= signedFDR))+
  ...
  geom_text(data=subset(sig_res), aes(label= formatC(fdr, format = "e", digits = 2)), color ="white", size = 1.5) +
  theme_minimal() +    
...
model.data %>% 
  mutate(quadrant = case_when(BASELINEcog_res > x_mid & IT > y_mid   ~ "High Cognitive Residual Score and High IT rCTPs",
                              BASELINEcog_res <= x_mid & IT > y_mid  ~ "Low Cognitive Residual Score and High IT rCTPs",
                              BASELINEcog_res <= x_mid & IT <= y_mid ~ "Low Cognitive Residual Score and Low IT rCTPs",
                              TRUE                                         ~ "High Cognitive Residual Score and Low IT rCTPs")) %>% 
  ggplot(aes(x = BASELINEcog_res, y = IT, color = quadrant)) +
...
```{r}
marker <- c("subclass_CgG_MTG_noSST")
setwd('./commonMarkerLists')
#get marker list
...
relevant_merge <- data.frame("projid"= mgps_ROSMAP$projid, "SST" = mgps_ROSMAP$SST, "IT"= mgps_ROSMAP$IT)
...
model_data_noSST$model <- gsub("IT", 'IT', model_data_noSST$model, fixed = T)
