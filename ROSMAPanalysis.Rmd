...
#get marker name
marker <- c("subclass_FINAL_common_final")
setwd('./commonMarkerLists')
#get marker list
subclass_FINAL_common_final <-  readRDS(paste0(marker, '.rds'))
setwd('../')
...
#calculate mgps
mgps_ROSMAP <- mgpCalc(count_df=mgp_df,markers= subclass_FINAL_common_final)
...
relevant_merge <- data.frame("projid"= mgps_ROSMAP$projid, "SST" = mgps_ROSMAP$SST, "L3.5.IT"= mgps_ROSMAP$L3.5.IT)
```
...
model_names <- c("BASELINE", "APOE4D","APOE2D", "L3.5.IT", "APOE4D.L3.5.IT", "APOE2D.L3.5.IT", "SST", "APOE4D.SST", "L3.5.IT.SST","APOE2D.SST")
...
for(model in model_names){
  ...
  else if (model == "L3.5.IT"){
    form <- paste("as.formula( mod_info$cogn_globaln_lv ~ ",
                  paste(append(base_vars,"L3.5.IT"), sep="",collapse=" + ")," )",sep="")
    
  }
  else if (model == "APOE4D.L3.5.IT"){
    form <- paste("as.formula( mod_info$cogn_globaln_lv ~ ",paste(append(base_vars, "apoe4d + L3.5.IT"), sep="",collapse=" + ")," )",sep="")
    
  }
  else if (model == "APOE2D.L3.5.IT"){
    form <- paste("as.formula( mod_info$cogn_globaln_lv ~ ",paste(append(base_vars,"apoe2d + L3.5.IT"), sep="",collapse=" + ")," )",sep="")
  ...
  else if (model == "L3.5.IT.SST"){
    form <- paste("as.formula( mod_info$cogn_globaln_lv ~ ",paste(append(base_vars, "L3.5.IT + SST"), sep="",collapse=" + ")," )",sep="")
...
```{r}
cell_type_names  <- (names(subclass_FINAL_common_final))
...
model_data$model <- gsub("L3+5+IT", 'L3.5.IT', model_data$model, fixed = T)
...
L3.5.IT_cogn <- ggplot(data=model.data, aes(y=cogn_globaln_lv, x=L3.5.IT)) + theme_minimal()+ geom_point() +
  geom_smooth(method=lm, se =F)  


L3.5.IT_tangles <- ggplot(data=model.data, aes(y=tangles_sqrt, x=L3.5.IT)) + theme_minimal()+ geom_point() +
  geom_smooth(method=lm, se =F)  

L3.5.IT_amyloid <- ggplot(data=model.data, aes(y=amyloid_sqrt, x=L3.5.IT)) + theme_minimal()+ geom_point() +
  geom_smooth(method=lm, se =F) 

mid_plots = plot_grid(SST_cogn, SST_amyloid, 
                     SST_tangles,L3.5.IT_cogn,
                     L3.5.IT_amyloid, L3.5.IT_tangles,  nrow = 2, ncol = 3,
...
Okay we want to visualize the L3.5.IT rCTPs and cognitive decline in a very simple graph.
```{r}

x_mid <- mean(c(max(model.data$BASELINEcog_res, na.rm = TRUE), 
                min(model.data$BASELINEcog_res, na.rm = TRUE)))

y_mid <- mean(c(max(model.data$L3.5.IT, na.rm = TRUE), 
                min(model.data$L3.5.IT, na.rm = TRUE)))

model.data %>% 
  mutate(quadrant = case_when(BASELINEcog_res > x_mid & L3.5.IT > y_mid   ~ "High Cognitive Residual Score and High L3.5.IT rCTPs",
                              BASELINEcog_res <= x_mid & L3.5.IT > y_mid  ~ "Low Cognitive Residual Score and High L3.5.IT rCTPs",
                              BASELINEcog_res <= x_mid & L3.5.IT <= y_mid ~ "Low Cognitive Residual Score and Low L3.5.IT rCTPs",
                              TRUE                                         ~ "High Cognitive Residual Score and Low L3.5.IT rCTPs")) %>% 
  ggplot(aes(x = BASELINEcog_res, y = L3.5.IT, color = quadrant)) +
  geom_vline(xintercept = x_mid) + # plot vertical line
  geom_hline(yintercept = y_mid) + # plot horizontal line
  geom_point()
```
Let's try binning the L3.5.IT rCTPs and visualizing boxplots
```{r}


quantile_IT<- transform(model.data, Q = cut(model.data$L3.5.IT, 
                              breaks = quantile(model.data$L3.5.IT, seq(0, 1, .2)), 
                              labels = c(1, 2, 3, 4, 5) ,
                              include.lowest=TRUE))
...
quantile_IT %>% 
  ...
  xlab('L3.5.IT Cell-Type Proportion Quintiles')  +
...
```{r}
marker <- c("subclass_FINAL_common_final_noSST")
setwd('./commonMarkerLists')
#get marker list
...
for(model in model_names){
  ...
  if(model == "BASELINE"){
    ...
    form <- paste("as.formula( mod_info$cogn_globaln_lv ~ ",paste(append(base_vars,"L3.5.IT"), sep="",collapse=" + ")," )",sep="")
...
model_data_noSST$model <- gsub("L3+5+IT", 'L3.5.IT', model_data_noSST$model, fixed = T)
