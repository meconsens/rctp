...
```{r}
marker_lists <- c("subclass_new", "subclass_MTGCgG_derived", "in_ex_CgG","subclass_CgG", "subclass_MTG", "lake", "darmanis")
...
marker_lists <- c("subclass_new", "subclass_MTGCgG_derived","in_ex_CgG","subclass_CgG", "subclass_MTG", "subclass_both", "lake", "darmanis")
...
```{r echo=FALSE, cache=FALSE, comment=FALSE, warning=FALSE}
marker_lists <- c("subclass_new", "subclass_MTGCgG_derived", "in_ex_CgG","subclass_CgG", "subclass_MTG", "subclass_both_CgG", "subclass_both_MTG", "lake", "darmanis")
for(markers in marker_lists){
    print(markers)
    setwd('./commonMarkerLists')
...
marker_lists <- c("subclass_new_common_final", "subclass_MTGCgG_derived_common_final", "in_ex_CgG_common_final","subclass_CgG_common_final", "subclass_MTG_common_final","subclass_both", "lake", "darmanis")
...
```{r}
cohorts <- c("ROSMAP", "MAYO", "BM10", "BM22", "BM36", "BM44")
marker_lists <- c("subclass_new_common_final","subclass_MTGCgG_derived_common_final","in_ex_CgG_common_final","subclass_CgG_common_final", "subclass_MTG_common_final","subclass_both", "lake", "darmanis")s
...
```
Let's get this done for the subclass_new marker list as well.
```{r}
subclass_new_no_SST <- subclass_new_common_final
subclass_new_no_SST$SST  <- subclass_new_no_SST$SST[-9]

setwd('./commonMarkerLists')
saveRDS(subclass_new_no_SST,'subclass_new_no_SST.rds')
setwd('../')
...
for(cohort in cohorts){
  mgp_result <- mgpsForAMPAD(cohort, subclass_new_no_SST)
  mgp_name <- paste0("mgp_",cohort)
  print(mgp_name)
  assign(mgp_name, mgp_result)
  setwd(paste0('./mgpResults_subclass_new_no_SST'))
  saveRDS(get(mgp_name), paste0(mgp_name, ".rds"))
  mgp_df <- mgp_result$model
  
  for(cell in names(subclass_new_no_SST)){
    mgp_df[,cell] <- as.numeric(scale(mgp_df[,cell], center = TRUE, scale = TRUE))
  }
  mgp_ZScored_name <- paste0(mgp_name, "_ZScored")
  assign(mgp_ZScored_name, mgp_df)
