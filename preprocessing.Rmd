...
createYDGEAMPAD <- function(cohort){
  setwd('./rawCohortData')
  if (cohort == "ROSMAP"){
    filename <- "ROSMAP_all_counts_matrix.txt.gz"
  }
  if(cohort =="MAYO"){
    filename <- "Mayo_TCX_all_counts_matrix.txt.gz"
  }
  if(cohort == "MSBB"){
    filename <- "MSSM_all_counts_matrix.txt.gz"
  }
  ...
  if(cohort == "ROSMAP"){
    ...
    pdata <- read.table("ageCensoredCovariates.tsv", sep="\t",header=T,check.names = F,stringsAsFactors = F)
    setwd('../')
    rownames(pdata) <- pdata$SampleID  
    subjects_to_include <- pdata$SampleID
  }
  if(cohort == "MAYO"){
    # read in phenotype/covariate data
    setwd('./cohortMetadata')
    pdata <- read.table("MAYO_CBE_TCX_Covariates.tsv", sep="\t",header=T,check.names = F,stringsAsFactors = F)
    setwd('../')
  ...
  if (cohort == "MSBB"){
    # read in phenotype/covariate data
    setwd('./cohortMetadata')
    pdata <- read.table("MSBB_RNAseq_covariates_November2018Update.csv", sep=",",header=T,check.names = F,stringsAsFactors = F)
    
    # "BM_22_", "BM_36_", "BM_10_", "BM_44" are 4 brodmann areas of msbb
    phenodata <- read.table("msbb_individual_metadata.csv", sep=",",header=T,check.names = F,stringsAsFactors = F)
    phenodata <- phenodata %>% 
      dplyr::rename(
    ...
    samples<- read.table("removesinai.txt", sep=",",header=T,check.names = F,stringsAsFactors = F)
    ...
    setwd('./rawCohortData')
    saveRDS(MSBBIDsAcross, "allMSBBIDs.rds")
    setwd('../')
  }
  if(cohort == "ROSMAP" | cohort=="MAYO"){
    ...
    setwd('./rawYDGE')
    saveRDS(ydge, file=paste0("raw_", cohort_ydge_name, ".rds"))
    setwd('../')
  }
  else{
    for(MSBB in MSBBs){
      ...
      setwd('./rawYDGE')
      saveRDS(ydge, file=paste0("raw_", region_ydge_name, ".rds"))
      setwd('../')
    }
...
for(cohort in cohorts){
  ...
  print(cohort)
  cohort_ydge_name <-  paste0(cohort, "ydge")
  filename <- paste0("raw_", cohort_ydge_name, ".rds")
  setwd('./rawYDGE')
...
QCpipelineAMPAD <- function(cohort){
  ...
  fileConn<-file(paste0(cohort,"QCresults.txt"))
  writeLines(
  ...
  assign(y_name, y2)
  print(y_name)
  saveRDS(y2,file=paste0(y_name, ".rds"))
  
  v_name <-  paste0("v_", cohort)
  v <- voom(y2)
  assign(v_name, v)
  print(v_name)
  saveRDS(v,file=paste0(v_name, ".rds"))
  setwd('../')
}
...
removeBatchAMPAD <- function(cohort){
  ...
  QC_filename <- paste0(cohort_QC_ydge_name, ".rds")
  voom_filename <- paste0("v_", cohort, ".rds")
  ### read in QCd data
  setwd('./QCpipelineResults')
  ...
  saveRDS(cohort_matrix, file=paste0(matrix_name, ".rds"))
  setwd('../')
  ...
  setwd('./cohortQCMods')
  saveRDS(lmod, file=paste0(lmod_name, ".rds"))
  
  eb <- eBayes(lmod,robust = T)
  eb_name <- paste0(cohort, "_eb")
  assign(eb_name, eb)
  saveRDS(eb,file=paste0(eb_name, ".rds"))
  ...
  saveRDS(tt,file=paste0(tt_name, ".rds"))
  setwd('../')
